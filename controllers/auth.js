...
export const login = async (req,res,next) => {
    try{
        ...
        if(!isPasswordCorrect) res.status(400).json({"message":"Datos de acceso incorrectos"})
        ...
        res.cookie("access_token",token,{
            httpOnly: true // para evitar que el frontend acceda al cookie desde javascript y pued
        }).status(200).json({...other,token})
    }catch(err){
      res.status(500).json({"erro":"No se logró"})
    }
}